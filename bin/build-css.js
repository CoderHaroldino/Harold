...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isVikingPackage(file) ?
    `${outputDir}/Viking.css` :
    `${outputDir}/style.css`;
...
function isVikingPackage(file) {
  return path.normalize(file).includes('packages/Viking/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@Viking/}*/src/style.scss');
