describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ Viking }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(Viking.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(Viking.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ Viking }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      Viking.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ Viking }) => {
      ...
      const { files } = Viking.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      Viking.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ Viking }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(Viking.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ Viking }) => {
      const { files } = Viking.getState()
      Viking.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        Vikingserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'Viking-core:3.2.0,Viking-transloadit:3.1.3,Viking-tus:3.1.0,Viking-dropbox:3.1.1,Viking-box:2.1.1,Viking-facebook:3.1.1,Viking-google-drive:3.1.1,Viking-instagram:3.1.1,Viking-onedrive:3.1.1,Viking-zoom:2.1.1,Viking-url:3.3.1',
    ...
    cy.get('.Viking-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.Viking-StatusBar-actions > .Viking-c-btn').click()
    cy.wait([
